...
def test_model(test_loader, nets, current_iter, tracker, writer, logger, save_this_iter):
    ...
    logger.info(f"Finished Testing! Mean Loss: {mean_test_loss}, Acc: {mean_test_acc}.")
    ...
    if save_this_iter:
        tracker.create_new_save('val_'+str(round(tracker.best_acc, 4))+'_test_'+str(
            round(mean_test_acc, 4))+'_'+str(current_iter+1).zfill(8)+'_real'+'.pth', nets[0])

        tracker.create_new_save('val_' + str(round(tracker.best_acc, 4)) + '_test_' + str(
            round(mean_test_acc, 4)) + '_' + str(current_iter + 1).zfill(8) + '_imag' + '.pth', nets[1])

        tracker.create_new_save('val_' + str(round(tracker.best_acc, 4)) + '_test_' + str(
            round(mean_test_acc, 4)) + '_' + str(current_iter + 1).zfill(8) + '_ph' + '.pth', nets[2])
...
def val_model(val_loader, nets, current_iter, tracker, writer, logger):
    ...
    logger.info(f"Finished Validation! Loss: {mean_val_loss}, Acc: {mean_val_acc}")
