...
class InfiniteMixturePrototype(nn.Module):
    ...
    def _add_cluster(self, nClusters, protos, radii, cluster_type='unlabeled', ex=None):
        ...
        d_radii = torch.ones(bsize, 1)

        if cluster_type == 'labeled':
            d_radii = d_radii * torch.exp(self.log_sigma_l.cpu())
        else:
        ...
        if ex is None:
            ...
            new_proto = ex.unsqueeze(0).unsqueeze(2)
    ...
    def _compute_protos(self, h, probs):
        ...
        """
        B, _, K = probs.shape
        # computes sum_{n}( h[b,n,c,d] * probs[b,n,k] )
        numerator = torch.einsum('bncd,bnk->bkcd', h, probs)

        # then divide by the per‐cluster sums
        protos = numerator / probs.sum(1).view(B, K, 1, 1)
    ...
    def dp_means_clustering(self, train_loader):
        ...
        all_lbls = torch.cat(all_lbls, dim=0).unsqueeze(0)

        # all_embs = all_embs.cuda()
        # self.all_lbls = all_lbls.unsqueeze(0).cuda()

        # self.nClusters = len(np.unique(all_lbls.data.cpu().numpy()))
        self.nClusters = len(np.unique(all_lbls.data))
        self.nInitialClusters = self.nClusters

        # _, idx = np.unique(self.all_lbls.data.cpu().numpy(), return_inverse=True)
        _, idx = np.unique(all_lbls.data, return_inverse=True)
        # prob_train = one_hot(self.all_lbls, self.nClusters).cuda()
        # prob_train = one_hot(all_lbls, self.nClusters).cuda()
        prob_train = one_hot(all_lbls, self.nClusters)

        # bsize = self.all_embs.size()[0]
        bsize = all_embs.size()[0]
        # self.radii = Variable(torch.ones(bsize, self.nClusters)).cuda() * torch.exp(self.log_sigma_l)
        self.radii = torch.ones(bsize, self.nClusters) * torch.exp(self.log_sigma_l.cpu())

        # self.support_labels = torch.arange(0, self.nClusters).cuda().long()
        self.support_labels = torch.arange(0, self.nClusters).long()

        # compute initial prototypes from labeled examples
        # self.protos = self._compute_protos(self.all_embs, prob_train)
        # self.protos = self._compute_protos(all_embs, prob_train).cuda()
        self.protos = self._compute_protos(all_embs, prob_train).transpose(2, 1)
        ...
        for i, ex in enumerate(all_embs[0]):
            # idxs = torch.nonzero(self.all_lbls.squeeze().data[i] == self.support_labels)[0]
            idxs = torch.nonzero(all_lbls.squeeze().data[i] == self.support_labels.cpu())[0]
            ...
            if (torch.min(distances) > lamda):
                ...
                self.nClusters, tensor_proto, self.radii = self._add_cluster(self.nClusters, tensor_proto, self.radii.cpu(),
                ...
                self.support_labels = torch.cat([self.support_labels.cpu(), all_lbls[0, i].reshape([1])], dim=0)
        ...
        if self.nClusters > self.nInitialClusters:
            ...
            support_targets = all_lbls[0, :, None] == self.support_labels.cpu()
            # prob_train = assign_cluster_radii_limited(Variable(tensor_proto), x, self.radii, support_targets)
            prob_train = assign_cluster_radii_limited(tensor_proto, all_embs, self.radii.cpu(), support_targets)
        ...
        self.protos = tensor_proto
        self.protos = self._compute_protos(self.all_embs, Variable(prob_train.data, requires_grad=False).cuda())
        # self.protos = self._compute_protos(all_embs, Variable(prob_train.data, requires_grad=False).cuda())
        # self.protos = self._compute_protos(all_embs, Variable(prob_train.data, requires_grad=False))
        self.protos, self.radii, self.support_labels = self.delete_empty_clusters(self.protos, prob_train,
                                                                                  self.radii.cpu(), self.support_labels.cpu())

    def forward(self, x, y, x_val=None, y_val=None, train_flag=True):

        # self.encoder.train()
        h = self.encoder(x.cuda())
        self.protos = self.protos.cuda()
        self.radii = self.radii.cuda()
        self.support_labels = self.support_labels.cuda()
